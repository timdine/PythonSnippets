...
def getGameRankAndPlays(aGame):
	print "link:   http://www.boardgamegeek.com/xmlapi2/thing?stats=1&id="+str(aGame.bggItemNumber)+"&thing=boardgame"
	r=requests.get("http://www.boardgamegeek.com/xmlapi2/thing?stats=1&id="+str(aGame.bggItemNumber)+"&thing=boardgame")
	...
	print(aGame.name, theGameRank, aGame.bggItemNumber)
	if (theGameRank == "Not Ranked"):
		theGameRank = -1
	aGame.rank = theGameRank

	print "link:   https://www.boardgamegeek.com/xmlapi2/plays?id="+str(aGame.bggItemNumber)+"&thing=boardgame"
	r=requests.get("https://www.boardgamegeek.com/xmlapi2/plays?id="+str(aGame.bggItemNumber)+"&thing=boardgame")
	while (r.status_code == 202):
		time.sleep(2)
		print("Got 202, retrying")
		r=requests.get("https://www.boardgamegeek.com/xmlapi2/plays?id="+str(aGame.bggItemNumber)+"&thing=boardgame")
...
for theGeekList in theGeeklists:
	...
	#Loop that attempts to get the XML again if the reported number of items is not the same as the item count that is returned
	while (itemNumbers != len(soup.find_all('item')) and loopcount < 1):
		...
		if len(soup.find_all('item')) > maxNumber:
			...
			maxNumberXML = soup
			loopcount = 0
	#end up with the XML that matches the most items returned by the request
	soup = maxNumberXML
	...
	for link in soup.find_all('item'):
		...
		try:
			...
			if theGameType == "boardgame" or theGameObjectType == "thread":
				...
				imageREWithoutCase = re.compile(r"\[ImageID=.*\]", re.IGNORECASE)
				imageList = imageREWithoutCase.findall(link.text)
				newImageList = []
				for item in imageList:
					newitem = simplifyImageLink(item)
	...
	if getRanks == 1:
		if foundGames[key].subtype == "boardgame":
			if foundGames[key].bggItemNumber in ranksDict.keys():
				...
				time.sleep(2)
				try:
					foundGames[key] = getGameRankAndPlays(foundGames[key])
...
for i in reversed(range(999)): #maxPlays
	for key, value in sorted(foundGames.iteritems(), key=operator.itemgetter(1)):
	#for key in foundGames.keys():
		if foundGames[key].builds == i and i >7:
